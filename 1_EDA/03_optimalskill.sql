...
-- we need to combines all theree tables using inner joins to use columns skill
SELECT
    sd.skills,
    ROUND(MEDIAN(jpf.salary_year_avg), 1) AS median_salary,
    COUNT(*) AS demand_count,
    ROUND(LN(COUNT(*)), 1) AS ln_demand_count,
    ROUND((LN(COUNT(*)) * MEDIAN(jpf.salary_year_avg)) / 1000000, 2) AS optimal_score
...
│ pandas     │      140000.0 │           98 │             4.6 │          0.64 │
│ word       │      140000.0 │           92 │             4.5 │          0.63 │
│ looker     │      136000.0 │           87 │             4.5 │          0.61 │
└────────────┴───────────────┴──────────────┴─────────────────┴───────────────┘
*/